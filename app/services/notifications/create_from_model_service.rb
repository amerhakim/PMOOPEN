...
class Notifications::CreateFromModelService
  ...
  def strategy
    # String, not a :"..." symbol -- Module#const_defined? raises
    # "NameError: wrong constant name" on a Symbol containing "::"
    # (e.g. a namespaced journable like VendorManagement::PurchaseOrder),
    # even though it happily resolves a qualified STRING path. Every
    # journable this method had been exercised against before was a
    # top-level class (Document/WorkPackage/News/...), so this never
    # surfaced. Confirmed both namespaced and flat lookups work correctly
