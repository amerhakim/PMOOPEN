...
module WorkPackages
  module Shared
    class WorkingDays
      ...
      # Like +with_lag+, but without the "successor cannot start the same
      # day the anchor date happens" baseline that Finish-to-Start
      # relations need. Here, +lag+ 0 means "the same working day as
      # +date+", +lag+ 2 means "2 working days after +date+", and so on.
      # Used for Start-to-Start / Finish-to-Finish / Start-to-Finish
      # relation kinds, where the anchor date is not "occupied" by the
      # predecessor the way a Finish-to-Start anchor is.
      def with_offset(date, lag)
        return nil unless date

        lag ||= 0
        return date if lag.zero?

        if lag.positive?
          shift_forward(date, lag)
        else
          shift_backward(date, -lag)
        end
      end
      ...
      def shift_forward(date, count)
        d = date
        while count.positive?
          d += 1
          count -= 1 if working?(d)
        end
        d
      end

      def shift_backward(date, count)
        d = date
        while count.positive?
          d -= 1
          count -= 1 if working?(d)
