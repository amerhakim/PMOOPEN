  ...
  module V3
    module Relations
      class RelationRepresenter < ::API::Decorators::Single
        ...
        ##
        # The MS-Project-style scheduling kind (FS/SS/FF/SF) for "precedes/follows"
        # relations. Always rendered as one of the concrete SCHEDULE_RELATION_TYPES
        # values (a `nil` in the database is normalized to "FS" on read), but a
        # `nil` payload is still accepted on write and stored as `nil` (== "FS").
        property :schedule_relation_type,
                 as: :scheduleRelationType,
                 exec_context: :decorator,
                 render_nil: true,
                 if: ->(*) {
                   [Relation::TYPE_FOLLOWS, Relation::TYPE_PRECEDES].include?(represented.relation_type) ||
                     represented.relation_type.blank?
                 }
        ...
        def schedule_relation_type
          represented.effective_schedule_relation_type
        end

        def schedule_relation_type=(value)
          represented.schedule_relation_type = value
        end
