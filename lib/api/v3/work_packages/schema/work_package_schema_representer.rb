    ...
    module WorkPackages
      module Schema
        class WorkPackageSchemaRepresenter < ::API::Decorators::SchemaRepresenter
          ...
          # Custom, non-AR-backed field: an editable, MS-Project-style predecessors
          # list ("3FS+2d, 7SS"). Value handling is entirely client-side (parses the
          # text and calls the relations API directly), so this schema entry exists
          # only to make the property show up as an editable field in the WP table.
          schema :predecessors,
                 type: "String",
                 name_source: ->(*) { "Predecessors" },
                 required: false,
                 writable: true
