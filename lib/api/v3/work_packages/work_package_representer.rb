  ...
  module V3
    module WorkPackages
      class WorkPackageRepresenter < ::API::Decorators::Single
        ...
        # Custom, non-AR-backed field: value handling for the editable
        # predecessors column happens entirely client-side via direct calls
        # to the relations API, so this property is a static placeholder
        # purely to satisfy the schema/edit-field machinery.
        ...
        # Companion to :predecessors above, same rationale.
        property :successors,
                 exec_context: :decorator,
                 getter: ->(*) { "" },
                 render_nil: true,
        ...
        # Read-only, purely date-based percentage: see
        # WorkPackages::ExpectedCompletion#expected_completion.
        property :expected_completion,
                 exec_context: :decorator,
                 getter: ->(*) { represented.expected_completion },
                 render_nil: true,
                 uncacheable: true
        ...
        def predecessors=(_value)
          # noop - handled via direct calls to the relations API from the frontend
        end

        def successors=(_value)
          # noop - handled via direct calls to the relations API from the frontend
        end
