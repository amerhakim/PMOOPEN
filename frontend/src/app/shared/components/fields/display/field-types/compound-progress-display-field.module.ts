...
export class CompoundProgressDisplayField extends DisplayField {
  ...
  public render(element:HTMLElement, displayText:string):void {
    ...
    // Per the user: %Complete on a work package with children should
    // show one plain percentage -- the rolled-up derived value -- not
    // the usual own-value/Σ-derived-value split every other compound
    // field (Work, Remaining work) still uses. Scoped to this one field
    // name so Work/Remaining work keep the normal split display.
    if (this.name === 'percentageDone' && this.hasChildren() && this.derivedValue !== null) {
      this.renderDerivedAsPlainValue(element);
      return;
    }
  ...
  private renderDerivedAsPlainValue(element:HTMLElement):void {
    const span = document.createElement('span');

    span.textContent = this.derivedValueString;
