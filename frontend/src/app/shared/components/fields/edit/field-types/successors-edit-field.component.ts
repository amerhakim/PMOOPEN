...
import {
  formatSuccessorsList,
  parsePredecessorsText,
  successorRelationsOf,
} from 'core-app/shared/components/fields/display/field-types/predecessors-format';

@Component({
  templateUrl: './successors-edit-field.component.html',
  standalone: false,
  // eslint-disable-next-line @angular-eslint/prefer-on-push-component-change-detection
  changeDetection: ChangeDetectionStrategy.Eager,
})
export class SuccessorsEditFieldComponent extends EditFieldComponent {
  public text = '';
  ...
  protected initialize():void {
    ...
    void this.wpRelations.require(this.workPackage.id!).then((relations) => {
      this.text = formatSuccessorsList(this.workPackage, relations);
    });
  }
  ...
  private async save():Promise<void> {
    ...
    // the initial creation. Successors typed here before the work package
    // exists can't be linked -- surface that rather than silently dropping
    // them or failing the whole form on the "involved" lookup below (which
    // requires a real, integer work package id).
    if (isNewResource(this.workPackage)) {
      if (desired.length > 0) {
        const message = 'Successors can only be added after the work package has been created.';
        this.errorMessage = message;
        this.handler.setErrors([message]);
    ...
    const current = successorRelationsOf(this.workPackage, relations);
    ...
    try {
      ...
      await Promise.all([
        ...
        ...toCreate.map((entry) => this.createSuccessor(entry.id, entry.scheduleRelationType, entry.lag)),
      ]);
    ...
    this.text = formatSuccessorsList(this.workPackage, refreshed);
  }

  // The current work package is the *predecessor* here (from/to are
  // swapped compared to createPredecessor). Critically, the relations API
  // endpoint always forces `from` to be the work package the URL is
  // nested under (see WorkPackageRelationsAPI's params_modifier), ignoring
  // whatever `_links.from` is sent in the payload. So to get from=successor,
  // to=this work package, we must POST to the *successor's* relations
  // endpoint, not this work package's own -- posting to our own endpoint
  // here would silently force from=to=this work package (a self-relation,
  // which the backend correctly rejects as a "circular dependency").
  private createSuccessor(successorId:string, scheduleRelationType:string, lag:number):Promise<RelationResource> {
    const params = {
      _links: {
        to: { href: this.apiV3Service.work_packages.id(this.workPackage.id!).toString() },
      },
      type: 'follows',
    ...
    const path = this.apiV3Service.work_packages.id(successorId).relations.toString();
