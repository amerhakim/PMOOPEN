...
export class WorkPackageSchemaProxy extends SchemaProxy {
  ...
  public isAttributeEditable(property:string):boolean {
    ...
    // Per the user: %Complete on a work package with children is always
    // the automatic rolled-up value (see WorkPackages::
    // DurationBasedEstimatedHours server-side) -- never hand-editable,
    // so it can't drift out of sync with the children it's derived from.
    if (property === 'percentageDone' && !!this.resource.children) {
      return false;
    }
